...
namespace {
   ...
   public:
      ...
      LogicalResult matchAndRewrite(mlir::subop::LookupOp lookupOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
         ...
         auto* context = getContext();
         auto indexType = rewriter.getIndexType();
         // todo: this may change basing on index design, now is an array for var-len data
         auto hnswType = util::RefType::get(context, mlir::util::RefType::get(context, rewriter.getI8Type()));
         // todo: 1 get index pointer in memory
         Value castedPointer = rewriter.create<mlir::util::GenericMemrefCastOp>(loc, util::RefType::get(context, TupleType::get(context, { hnswType, indexType })), adaptor.getState());
   ...
   class CreateHnswIndexedViewLowering : public SubOpConversionPattern<mlir::subop::CreateHnswIndexedView> {
      using SubOpConversionPattern<mlir::subop::CreateHnswIndexedView>::SubOpConversionPattern;
   ...
   void SubOpToControlFlowLoweringPass::runOnOperation() {
      ...
      typeConverter.addConversion([&](mlir::subop::HnswIndexedViewType t) -> Type {
         return mlir::util::RefType::get(t.getContext(), mlir::IntegerType::get(ctxt, 8));
         });
      ...
      //HnswIndexedView
      rewriter.insertPattern<CreateHnswIndexedViewLowering>(typeConverter, ctxt);
      rewriter.insertPattern<LookupHnswIndexedViewLowering>(typeConverter, ctxt);
